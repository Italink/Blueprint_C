...
bool FBlueprint_C::IsAutoGenerateEnabled(UBlueprint* InBlueprint)
{
	if (InBlueprint == nullptr)
		return false;
	return InBlueprint->HideCategories.Contains("Blueprint_C");
}

void FBlueprint_C::SetAutoGenerateEnabled(UBlueprint* InBlueprint, bool bEnabled)
{
	if (bEnabled && !InBlueprint->HideCategories.Contains("Blueprint_C")) {
		InBlueprint->Modify();
		InBlueprint->HideCategories.Add("Blueprint_C");
	}
	else if (!bEnabled && InBlueprint->HideCategories.Contains("Blueprint_C")) {
		InBlueprint->Modify();
		InBlueprint->HideCategories.Remove("Blueprint_C");
	}
}

void FBlueprint_C::TryAutoGenerate(UBlueprint* InBlueprint, TPair<FString, FString> Code)
{
	if (FBlueprint_C::IsAutoGenerateEnabled(InBlueprint)) {
		FString PathName = InBlueprint->GetPackage()->GetPathName();
		TArray<FString> SplitPath;
		PathName.ParseIntoArray(SplitPath, TEXT("/"));
		FString ModuleName = SplitPath.IsEmpty() ? "Game" : SplitPath[0];
		if (Code.Value.IsEmpty() && ModuleName == "Game") {
			FString SubPath = PathName.RightChop( ModuleName.Len() + 2 + (SplitPath[1] == FApp::GetProjectName() ? SplitPath[1].Len() + 1 : 0));
			FString Path = FString::Printf(TEXT("%s%s/%s.inl")
				, *FPaths::ConvertRelativePathToFull(FPaths::GameSourceDir())
				, FApp::GetProjectName()
				, *SubPath
			);
			if (!Code.Key.IsEmpty()) {
				Code.Key = "/* This file is generated by Blueprint_C */ \n\n" + Code.Key;
				FFileHelper::SaveStringToFile(Code.Key, *Path);
			}
		}
		else {
			UE_LOG(LogTemp, Warning, TEXT("Please Customize"));
		}
	}
}